...
def centroid_similarity_combined_roles(df_sent: pd.DataFrame, embeddings: np.ndarray, roles_a: List[str], roles_b: List[str]) -> float:
...
def analyze_single_abstract(
    ...
    labels, confidences, raw_response = label_sentences_with_llm(
        sentences, client=openai_client, model_name=llm_model_name, verbose=verbose
    )
    ...
    embeddings = embedding_model.encode(df_sent["sentence"].tolist(), normalize_embeddings=True)
    ...
    threshold = np.nanmean(df_sent["local_similarity_to_previous_sentence"]) - np.nanstd(df_sent["local_similarity_to_previous_sentence"])
    df_sent["low_local_similarity_flag"] = df_sent["local_similarity_to_previous_sentence"] < threshold
    df_sent["within_role_low_similarity_flag"] = df_sent["low_local_similarity_flag"] & df_sent["same_role_as_previous"]
    ...
    summary["conclusion_to_background_plus_aim_alignment"] = centroid_similarity_combined_roles(df_sent, embeddings, ["Conclusion"], ["Background", "Aim"])
    summary["conclusion_plus_future_to_background_plus_aim_alignment"] = centroid_similarity_combined_roles(df_sent, embeddings, ["Conclusion", "Limitation/Future"], ["Background", "Aim"])
